    ...
    <div>
              ...
              <div key={label}>
                <div className="flex-between" style={{ marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 'var(--text-sm)', fontWeight: 600 }}>
                    {CONTRIBUTION_BY_CATEGORY_DATA.data[i]}%
                  </span>
                </div>
        ...
        >
          ...
          <div style={{ height: 180 }}>
            <svg viewBox="0 0 700 180" style={{ width: '100%', height: '100%' }}>
              ...
              {(() => {
                ...
                const pts = data
                  .map((v, i) => `${(i / (data.length - 1)) * 700},${170 - (v / max) * 160}`)
                  .join(' ');
                return (
                  <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            {MEMBER_GROWTH_DATA.labels.map((l) => (
              <span key={l} style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>
                {l}
              </span>
            ))}
          </div>
      ...
      <div className="grid grid-4 animate-fade-in-up stagger-4">
        {[
          ...
          >
            <span style={{ display: 'block', marginBottom: 'var(--space-sm)' }}>
              <OnlineLogoIcon name={s.icon} size={32} />
            </span>
            <p
              style={{
                fontFamily: 'var(--font-display)',
