    ...
    <div>
      <div className="page-header animate-fade-in-up">
        <div className="flex-between">
          <div>
            <h1>Announcements</h1>
            <p>Manage ministry announcements</p>
          </div>
            ...
            <div key={ann.id} className="glass-card-static" style={{ padding: 'var(--space-lg)' }}>
              <div className="flex-between" style={{ marginBottom: 'var(--space-sm)' }}>
                ...
                <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  <span
                    className={`badge ${ann.priority === 'high' ? 'badge-red' : ann.priority === 'normal' ? 'badge-blue' : 'badge-gray'}`}
                  >
                    {ann.priority}
                  </span>
                  <button className="btn btn-ghost btn-sm" onClick={() => openEdit(ann)}>
              ...
              >
                {ann.description.substring(0, 80)}...
              </p>
              <div className="flex-between">
      ...
      {announcements.length === 0 && (
        <div className="empty-state">
          <div className="empty-state-icon">
            <OnlineLogoIcon name="megaphone" size={48} color="var(--gold)" />
          </div>
              ...
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700 }}>
                {modal === 'create' ? (
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <OnlineLogoIcon name="edit" size={16} /> Edit Announcement
                  </span>
                )}
              </h3>
            ...
            <div className="modal-body">
              ...
              >
                <div className="input-group">
                  ...
                  >
                    {['News', 'Events', 'Fundraising', 'Prayer', 'Youth', 'Other'].map((c) => (
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {ICONS.map((icon) => (
                    <button
                      key={icon}
                      type="button"
                      onClick={() => setEditData((p) => ({ ...p, image: icon }))}
                      style={{
                        width: 32,
                        height: 32,
                        ...
                        background:
                          editData.image === icon ? 'rgba(212,168,67,0.1)' : 'var(--bg-secondary)',
                        cursor: 'pointer',
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      <OnlineLogoIcon name={icon} size={16} />
                    </button>
                  ))}
                </div>
              </div>
