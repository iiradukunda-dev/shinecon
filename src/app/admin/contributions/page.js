...
export default function AdminContributionsPage() {
  const { 
    contributions, approveContribution, rejectContribution, deleteContribution,
  ...
  const [typeFormData, setTypeFormData] = useState({
    name: '', description: '', category: 'Regular', 
    localStudent: 0, localEmployed: 0, diasporaStudent: 0, diasporaEmployed: 0,
    currency: 'RWF', recurring: false, active: true, icon: 'wallet', color: '#D4A843'
  });
  ...
  const handleOpenTypeModal = (type = null) => {
    if (type) {
      ...
      setTypeFormData({
        name: type.name, description: type.description, category: type.category,
        localStudent: type.localStudent, localEmployed: type.localEmployed,
        diasporaStudent: type.diasporaStudent, diasporaEmployed: type.diasporaEmployed,
        currency: type.currency, recurring: type.recurring, active: type.active,
  ...
  return (
    <div>
      ...
      {/* Main Tabs */}
      <div className="tab-container animate-fade-in-up stagger-1" style={{ marginBottom: 'var(--space-xl)' }}>
        <button 
                  ...
                  <tr key={c.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        {c.status === 'pending' && (
                          <>
                            <button className="btn btn-gold btn-sm" onClick={() => approveContribution(c.id)}>✓</button>
      ...
      {activeTab === 'types' && (
        <div className="animate-fade-in">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-lg)' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 700 }}>Contribution Types</h2>
          ...
          <div className="grid grid-3">
            {contributionTypes.map(ct => (
              <div key={ct.id} className="glass-card-static" style={{ padding: 'var(--space-lg)', position: 'relative' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-md)' }}>
                  <div style={{
                    width: 48, height: 48, borderRadius: 'var(--radius-lg)',
                    background: `${ct.color}22`, display: 'flex', alignItems: 'center',
                    justifyContent: 'center', color: ct.color
                  }}>
                    <OnlineLogoIcon name={ct.icon || 'wallet'} size={24} />
                  </div>
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button className="btn btn-ghost btn-sm" onClick={() => handleOpenTypeModal(ct)}><OnlineLogoIcon name="edit" size={16} /></button>
                    <button className="btn btn-ghost btn-sm" onClick={() => setDeleteTypeTarget(ct)} style={{ color: 'var(--soft-red)' }}><OnlineLogoIcon name="trash-2" size={16} /></button>
                  </div>
                </div>
                <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, marginBottom: 4 }}>{ct.name}</h3>
                <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', marginBottom: 'var(--space-md)' }}>{ct.description}</p>
                ...
                <div style={{ fontSize: 'var(--text-xs)', background: 'var(--bg-tertiary)', padding: '8px', borderRadius: 'var(--radius-sm)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ color: 'var(--text-tertiary)' }}>Local (Student/Emp)</span>
                    <span style={{ fontWeight: 600 }}>{ct.localStudent} / {ct.localEmployed} {ct.currency}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-tertiary)' }}>Diaspora (Student/Emp)</span>
                    <span style={{ fontWeight: 600 }}>{ct.diasporaStudent} / {ct.diasporaEmployed} {ct.currency}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 600 }}>
            <form onSubmit={handleSaveType}>
              <div className="modal-header">
                ...
                <button type="button" className="btn btn-ghost" onClick={() => setShowTypeModal(false)}>✕</button>
              </div>
              <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                <div className="grid grid-2" style={{ gap: 'var(--space-md)' }}>
                  <div style={{ gridColumn: '1 / -1' }}>
                    <label className="input-label">Name</label>
                    <input className="input" required value={typeFormData.name} onChange={e => setTypeFormData({...typeFormData, name: e.target.value})} placeholder="e.g. Monthly Contribution" />
                  </div>
                  <div style={{ gridColumn: '1 / -1' }}>
                    <label className="input-label">Description</label>
                    <textarea className="input" value={typeFormData.description} onChange={e => setTypeFormData({...typeFormData, description: e.target.value})} placeholder="Brief description..." />
                  </div>
                  <div>
                    <label className="input-label">Category</label>
                    <input className="input" value={typeFormData.category} onChange={e => setTypeFormData({...typeFormData, category: e.target.value})} placeholder="e.g. Regular, Project" />
                  </div>
                  <div>
                    <label className="input-label">Currency</label>
                    <input className="input" value={typeFormData.currency} onChange={e => setTypeFormData({...typeFormData, currency: e.target.value})} placeholder="e.g. RWF, USD" />
                  </div>
                  
                  <div style={{ gridColumn: '1 / -1' }}>
                    <h4 style={{ margin: 'var(--space-sm) 0', fontSize: 'var(--text-sm)', color: 'var(--gold)' }}>Amounts</h4>
                  </div>
                  <div>
                    <label className="input-label">Local Student</label>
                    <input type="number" className="input" required value={typeFormData.localStudent} onChange={e => setTypeFormData({...typeFormData, localStudent: Number(e.target.value)})} />
                  </div>
                  <div>
                    <label className="input-label">Local Employed</label>
                    <input type="number" className="input" required value={typeFormData.localEmployed} onChange={e => setTypeFormData({...typeFormData, localEmployed: Number(e.target.value)})} />
                  </div>
                  <div>
                    <label className="input-label">Diaspora Student</label>
                    <input type="number" className="input" required value={typeFormData.diasporaStudent} onChange={e => setTypeFormData({...typeFormData, diasporaStudent: Number(e.target.value)})} />
                  </div>
                  <div>
                    <label className="input-label">Diaspora Employed</label>
                    <input type="number" className="input" required value={typeFormData.diasporaEmployed} onChange={e => setTypeFormData({...typeFormData, diasporaEmployed: Number(e.target.value)})} />
                  </div>
                  
                  <div style={{ gridColumn: '1 / -1' }}>
                    <h4 style={{ margin: 'var(--space-sm) 0', fontSize: 'var(--text-sm)', color: 'var(--gold)' }}>Settings</h4>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input type="checkbox" id="recurring" checked={typeFormData.recurring} onChange={e => setTypeFormData({...typeFormData, recurring: e.target.checked})} />
                    <label htmlFor="recurring" style={{ fontWeight: 500 }}>Recurring (Monthly)</label>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input type="checkbox" id="active" checked={typeFormData.active} onChange={e => setTypeFormData({...typeFormData, active: e.target.checked})} />
                    <label htmlFor="active" style={{ fontWeight: 500 }}>Active (Visible to members)</label>
                  </div>
                </div>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setShowTypeModal(false)}>Cancel</button>
                <button type="submit" className="btn btn-gold">Save Type</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {deleteTypeTarget && (
        <div className="modal-overlay" onClick={() => setDeleteTypeTarget(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 400 }}>
            <div className="modal-header">
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--soft-red)', display: 'flex', alignItems: 'center', gap: 8 }}><OnlineLogoIcon name="alert-triangle" size={16} color="var(--soft-red)" /> Delete Type</h3>
              <button className="btn btn-ghost" onClick={() => setDeleteTypeTarget(null)}>✕</button>
            </div>
            <div className="modal-body" style={{ textAlign: 'center' }}>
              <p>Are you sure you want to delete <strong>{deleteTypeTarget.name}</strong>?</p>
              <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', marginTop: 4 }}>This will remove it from member options. Past contributions of this type will also be deleted.</p>
            </div>
            <div className="modal-footer">
              <button className="btn btn-secondary" onClick={() => setDeleteTypeTarget(null)}>Cancel</button>
              <button className="btn btn-danger" onClick={handleDeleteType}>Delete</button>
            </div>
          </div>
        </div>
      )}
