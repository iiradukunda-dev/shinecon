  ...
  return (
    <div className="layout-root admin-layout">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`} style={{ top: 0 }}>
          ...
          {NAV_SECTIONS.map((section) => (
            <div key={section.label}>
              <div className="sidebar-section">
                {t(section.label.toLowerCase().replace(' ', '_'), language)}
              </div>
              {section.items.map((item) => {
                let badgeCount = item.badge;
                if (item.label === 'Members' && stats?.pendingMembers > 0)
                  badgeCount = stats.pendingMembers;
                if (item.label === 'Contributions' && stats?.pendingContributions > 0)
                  badgeCount = stats.pendingContributions;
        ...
        <div style={{ padding: 'var(--space-md)', borderTop: '1px solid var(--border-light)' }}>
          <button
            ...
            style={{ width: '100%', color: 'var(--soft-red)' }}
          >
            <span className="link-icon">
              <OnlineLogoIcon name="log-out" />
            </span>
            Logout
          </button>
        </div>
      ...
      <header className={`topbar ${isScrolled ? 'scrolled' : ''}`} style={{ top: 0 }}>
        ...
        <div className="topbar-right">
          <div className="topbar-search">
            <span>
              <OnlineLogoIcon name="search" size={16} color="var(--text-tertiary)" />
            </span>
            <input
      ...
      <main className="page-admin" style={{ paddingTop: `calc(var(--topbar-height) + 28px)` }}>
        <div className="page-admin-content">{children}</div>
      </main>
    </div>
  );
}
