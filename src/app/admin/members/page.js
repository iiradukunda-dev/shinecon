...
export default function AdminMembersPage() {
  const { members, addMember, updateMember, deleteMember, approveMember, rejectMember, addToast } = useApp();
  ...
  const openCreate = () => { setEditData({ ...EMPTY_MEMBER }); setModal('create'); };
  const openEdit = (m) => { setEditData({ ...m }); setModal('edit'); };
  const openDelete = (m) => { setDeleteTarget(m); setModal('delete'); };

  const handleSave = () => {
    if (!editData.name || !editData.email) { addToast('Name and email are required', 'error'); return; }
  ...
  return (
    <div>
      <div className="page-header animate-fade-in-up">
        <div className="flex-between">
          <div>
            ...
            <p>{members.length} total members • {members.filter(m => m.status === 'pending').length} pending approval</p>
          </div>
      ...
      {/* Filters */}
      <div className="flex-between animate-fade-in-up stagger-1" style={{ marginBottom: 'var(--space-lg)', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
        <div style={{ display: 'flex', gap: 4, background: 'var(--glass-bg)', borderRadius: 'var(--radius-full)', padding: 4 }}>
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>{m.employment}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <p style={{ fontSize: 'var(--text-sm)' }}>{m.email}</p>
                  <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>{m.phone}</p>
                </td>
                <td style={{ fontSize: 'var(--text-sm)' }}>{m.country}</td>
                <td><span className={`badge ${m.type === 'local' ? 'badge-gold' : 'badge-blue'}`}>{m.type}</span></td>
            ...
            <div className="modal-body">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 'var(--space-sm)' }}>
                <div className="input-group">
                  ...
                  <select className="select" value={editData.country} onChange={e => setEditData(p => ({ ...p, country: e.target.value }))}>
                    {['Rwanda', 'Belgium', 'Canada', 'France', 'Germany', 'United Kingdom', 'United States', 'Other'].map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
                <div className="input-group">
                  <label className="input-label">Type</label>
                  <select className="select" value={editData.type} onChange={e => setEditData(p => ({ ...p, type: e.target.value }))}>
                ...
                <div className="input-group">
                  ...
                  <select className="select" value={editData.employment} onChange={e => setEditData(p => ({ ...p, employment: e.target.value }))}>
