          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="flex-between">
                <span style={{ fontWeight: m.unread ? 700 : 500, fontSize: 'var(--text-sm)' }}>
                  {m.from}
                </span>
      ...
      {messages.length === 0 && (
        <div className="empty-state">
          <div className="empty-state-icon">
            <OnlineLogoIcon name="message-square" size={32} />
          </div>
          <p className="empty-state-title">No messages</p>
          <p className="empty-state-description">All clear!</p>
        </div>
      )}
    </div>
  );
