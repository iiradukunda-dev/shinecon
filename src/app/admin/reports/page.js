  ...
  const reportConfigs = [
    {
      ...
      getData: () => {
        const headers = ['Transaction ID', 'Member Name', 'Member Email', 'Contribution Type', 'Amount', 'Currency', 'Status', 'Date'];
        const rows = contributions.map(c => [
          c.id, c.memberName, c.memberEmail || 'N/A', c.type, c.amount, c.currency, c.status, c.date
        ]);
    ...
    },
    {
      ...
      description: 'Complete list of all registered members with contact details, location, and status.',
      category: 'Membership',
      format: ['CSV', 'Excel'],
    ...
    },
    {
      ...
      description: 'Progress, fundraising goals, raised totals, and donor metrics across campaigns.',
      category: 'Fundraising',
      format: ['CSV', 'PDF'],
      icon: <IconTarget size={24} color="#40C057" />,
      getData: () => {
        const headers = ['Campaign ID', 'Title', 'Goal Amount', 'Raised Amount', 'Currency', 'Contributors', 'Status', 'Deadline'];
        const rows = campaigns.map(c => [
          c.id, c.title, c.goal, c.raised, c.currency, c.contributors, c.status, c.deadline || 'Ongoing'
        ]);
    ...
    },
    {
      ...
      description: 'Executive financial summary aggregating revenue totals by currency and category.',
      category: 'Executive',
      format: ['CSV', 'PDF'],
    ...
    },
    {
      ...
      getData: () => {
        const headers = ['Log ID', 'Timestamp', 'User', 'Role', 'Action Executed', 'IP Address', 'Status'];
        const rows = [
          ['LOG-101', new Date().toISOString(), 'System Admin', 'admin', 'Database Backup & Sync', '192.168.1.1', 'SUCCESS'],
          ['LOG-102', new Date(Date.now() - 3600000).toISOString(), 'Jean-Pierre Habimana', 'member', 'Contribution Submitted', '197.243.0.12', 'SUCCESS'],
          ['LOG-103', new Date(Date.now() - 7200000).toISOString(), 'System Admin', 'admin', 'Member Approved', '192.168.1.1', 'SUCCESS'],
          ['LOG-104', new Date(Date.now() - 14400000).toISOString(), 'Marie Rose', 'member', 'Profile Updated', '197.243.2.45', 'SUCCESS'],
        ];
            ...
            <div>
              <div className="flex-between" style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: 'rgba(212,168,67,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
                    {report.icon}
                  </div>
                  <div>
              ...
              <div className="flex-between" style={{ padding: '12px 16px', background: 'rgba(255, 255, 255, 0.04)', borderRadius: 14, border: '1px solid rgba(255, 255, 255, 0.08)' }}>
                <span style={{ fontSize: 13, color: 'rgba(255, 255, 255, 0.6)' }}>Supported Formats:</span>
                <div style={{ display: 'flex', gap: 6 }}>
                  {report.format.map(fmt => (
                    <span key={fmt} className="badge badge-gray" style={{ fontSize: 11, color: '#D4A843' }}>
                      {fmt}
                    </span>
                  ))}
                </div>
          ...
          <div className="modal" style={{ maxWidth: 900, width: '92vw' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 22, color: '#D4A843' }}>
                    {previewData.title}
                  </h2>
                  <p style={{ fontSize: 13, color: 'rgba(255, 255, 255, 0.6)' }}>
                    Generated live on {new Date().toLocaleDateString()} • {previewData.rows.length} Total Records
                  </p>
                </div>
              </div>
              <button className="btn btn-ghost" style={{ fontSize: 20, color: '#FFFFFF' }} onClick={() => setPreviewData(null)}>
                ✕
              </button>
            </div>
            ...
            <div className="modal-body" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
              <div className="table-container">
                <table className="table">
                  ...
                  <tbody>
                    {previewData.rows
                      .filter(row => row.some(cell => String(cell).toLowerCase().includes(searchQuery.toLowerCase())))
                      .map((row, i) => (
                        <tr key={i}>
                          {row.map((cell, j) => (
                            <td key={j}>
                              {typeof cell === 'number' && j >= 4 ? formatCurrency(cell, 'RWF') : String(cell)}
                            </td>
                          ))}
                        </tr>
                      ))}
