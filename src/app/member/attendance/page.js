...
export default function AttendancePage() {
  ...
  const handleOpenScanner = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        setLocationError(
          'Location permission denied or unavailable. You must enable location to check in.',
        );
        setScanning(false);
      },
    );
  ...
  const simulateScan = async (type) => {
    ...
    if (distance > 200) {
      setScanError(
        `Location mismatch. You are ${Math.round(distance)}m away from the event location.`,
      );
      return;
    }
  ...
  return (
    <div className="page-member-content">
      ...
      {/* QR Scanner */}
      <div
        className="glass-card-static animate-fade-in-up stagger-1"
        style={{
      ...
      >
          ...
          <div className="animate-scale-in" style={{ padding: 'var(--space-xl)' }}>
            ...
            >
              ...
              <div style={{ opacity: 0.3 }}>
                <OnlineLogoIcon name="smartphone" size={48} />
              </div>
            </div>
            <p style={{ fontWeight: 600 }}>
              Scanning for <strong>{selectedAttendanceEvent?.event}</strong>...
            </p>
            ...
            {!userLocation ? (
              ...
              <p style={{ fontSize: 'var(--text-sm)', color: 'var(--emerald)' }}>
                Location acquired.
              </p>
            )}
        ...
        {scanned && (
          <div className="animate-scale-in" style={{ padding: 'var(--space-xl)' }}>
            <div
              style={{ marginBottom: 'var(--space-md)', display: 'flex', justifyContent: 'center' }}
            >
              <OnlineLogoIcon name="check-circle" size={64} color="var(--emerald)" />
            </div>
            <h3
              style={{
            ...
            >
              <p style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <OnlineLogoIcon name="calendar" size={14} /> {new Date().toLocaleDateString()}
              </p>
              <p style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '4px 0' }}>
                <OnlineLogoIcon name="clock" size={14} /> {new Date().toLocaleTimeString()}
              </p>
              <p style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <OnlineLogoIcon name="map-pin" size={14} /> Location verified ✓
              </p>
            </div>
            <button
